/** Playbook de Closer — guia completo para o time de Closers da Cardápio Web.
 *  Estrutura espelhada do guia de closers. Conteúdo hardcoded em
 *  '@/data/playbookCloser'. Bloco 1 ativo: Início, Planos & Preços,
 *  Franquias e Objeções. Demais abas em construção. */
import { useState } from 'react';
import {
  Compass, DollarSign, Building2, ShieldAlert, ClipboardList, Video,
  Gauge, MessageSquareReply, Filter, GraduationCap, Clock, Swords,
  ChevronDown, ChevronRight, Search, Copy, Check, Star,
} from 'lucide-react';
...
const TABS = [
  { id: 'inicio',       label: 'Início',                icon: Compass,           pronto: true,  desc: 'Encontre rapidamente qualquer conteúdo do playbook.' },
  { id: 'planos',       label: 'Planos e Preços',       icon: DollarSign,        pronto: true,  desc: 'Planos oficiais, módulos extras e cupons de desconto.' },
  { id: 'franquias',    label: 'Franquias',             icon: Building2,         pronto: true,  desc: 'Condições especiais e tabelas de desconto por volume.' },
  { id: 'objecoes',     label: 'Objeções',              icon: ShieldAlert,       pronto: true,  desc: 'Base de objeções organizada por fase da venda.' },
  { id: 'checklists',   label: 'Checklists de processos', icon: ClipboardList,   pronto: false, desc: 'Passo a passo para cada momento da operação.' },
  { id: 'etapas',       label: 'Etapas da reunião',     icon: Video,             pronto: false, desc: 'Guia detalhado das etapas durante a reunião.' },
  { id: 'criterios',    label: 'Critérios de avaliação', icon: Gauge,            pronto: false, desc: 'Matriz de avaliação do closer.' },
  { id: 'followup',     label: 'Follow-up',             icon: MessageSquareReply, pronto: false, desc: 'Scripts e fluxos para pós-reunião.' },
  { id: 'funis',        label: 'Funis de Vendas',       icon: Filter,            pronto: false, desc: 'Estrutura dos funis utilizados pelo time.' },
  { id: 'spin',         label: 'SPIN',                  icon: MessageSquareReply, pronto: false, desc: 'Metodologia SPIN com exemplos práticos.' },
  { id: 'progressao',   label: 'Progressão',            icon: GraduationCap,     pronto: false, desc: 'Os níveis de carreira e requisitos para promoção.' },
  { id: 'horaouro',     label: 'Hora Ouro',             icon: Clock,             pronto: false, desc: 'Rotinas de alta produtividade e checklists do CRM.' },
  { id: 'concorrentes', label: 'Concorrentes',          icon: Swords,            pronto: false, desc: 'Comparativo com os principais concorrentes.' },
] as const;
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
      ...
      <p className="text-sm text-cw-muted max-w-xs leading-relaxed">
        A aba <strong>{titulo}</strong> está sendo preparada e chega em breve.
      </p>
    </div>
  );
}
...
function Inicio({ onGo }: { onGo: (id: string) => void }) {
  return (
    <div className="space-y-6">
      <div className="cw-card p-5">
        <SectionTitle>Menu inicial</SectionTitle>
        <p className="text-sm text-cw-muted">Encontre rapidamente qualquer conteúdo do playbook. Clique numa seção para abrir.</p>
      </div>
        ...
        {TABS.filter(t => t.id !== 'inicio').map(t => {
          ...
          return (
            <button
              key={t.id}
              onClick={() => onGo(t.id)}
              className="cw-card cw-card-hover p-4 text-left flex items-start gap-3 group"
            >
              <div className="h-10 w-10 rounded-xl bg-cw-purple/10 border border-cw-purple/20 flex items-center justify-center shrink-0">
                <Icon className="h-5 w-5 text-cw-purple" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="font-bold text-sm text-cw-text">{t.label}</p>
                  {!t.pronto && (
                    <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-cw-elevated text-cw-muted border border-cw-border">em breve</span>
                  )}
                </div>
                <p className="text-xs text-cw-muted mt-0.5 leading-relaxed">{t.desc}</p>
              </div>
              <ChevronRight className="h-4 w-4 text-cw-muted shrink-0 mt-1 group-hover:translate-x-0.5 transition-transform" />
            </button>
          );
        })}
      </div>
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-cw-muted mt-1">Cardápio Web — guia completo para o time de Closers.</p>
      </div>
      ...
      <Tabs value={tab} onValueChange={setTab} className="w-full">
        ...
        <TabsContent value="inicio" className="mt-6"><Inicio onGo={setTab} /></TabsContent>
        <TabsContent value="planos" className="mt-6"><Planos /></TabsContent>
        <TabsContent value="franquias" className="mt-6"><Franquias /></TabsContent>
        <TabsContent value="objecoes" className="mt-6"><Objecoes /></TabsContent>

        {TABS.filter(t => !t.pronto).map(t => (
          <TabsContent key={t.id} value={t.id} className="mt-6"><EmBreve titulo={t.label} /></TabsContent>
        ))}
      </Tabs>
    </div>
  );
